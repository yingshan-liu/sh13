   ...
   ret = pthread_create ( & thread_serveur_tcp_id, NULL, fn_serveur_tcp, NULL);
	...
	if (SDL_PollEvent(&event))
	{
		...
		switch (gbuffer[0])
		{
			...
			int pla_current;
			sscanf(gbuffer,"M %d",&pla_current);
			/*if(pla_current = gId)
				goEnabled = 1;
			else
				goEnabled = 0;
			...
			case 'V':{
				// RAJOUTER DU CODE ICI
			int pla/*player*/,obj/*objet*/,nb/*number*/;
			sscanf(gbuffer,"V %d %d %d",&pla,&obj,&nb);
			printf("V %d %d %d\r\n", pla, obj, nb);
			tableCartes[pla][obj] = nb;
			printf("V Joueur[%d] = Carte[%d] : %d\r\n", pla, obj, tableCartes[pla][obj]);
