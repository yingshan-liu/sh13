...
int main(int argc, char *argv[])
{
     ...
     int n;
	int i;
    int pla_out[4] = {-1,-1,-1,-1};//whether the player is fail
        ...
        if (fsmServer==0)
        {
        	switch (buffer[0])
        	{
                                ...
                                if (nbClients==4)
				{
                    ...
                    for(int k=0;k<8;k++)
                    {
                     sprintf(reply,"V %d %d %d", 0, k, tableCartes[0][k]);//(nb player-NO.cart-nb cart)
                     sendMessageToClient(tcpClients[0].ipAddress, tcpClients[0].port, reply);
                    }
	...
	else if (fsmServer==1)
	{
		...
		switch (buffer[0])
		{
            ...
            if(deck[12]==guiltSel)
                {
                    sprintf(reply,"M %d",-1);//end,gId is winner
                    broadcastMessage(reply);
                    sprintf(reply,"\nThe resulat is: %s\nThe winner is:%s\n",nomcartes[guiltSel],tcpClients[gId].name);
                    broadcastMessage(reply);
                    fsmServer=0;//game is over with a winner.
                }
            else
                {
                    pla_out[gId] = 1;
                    while(1)
                    {
                        if((pla_out[0] == 1)&&(pla_out[1] == 1)&&(pla_out[2] == 1)&&(pla_out[3] == 1))
                        {
                            sprintf(reply,"M %d",-1);
                            broadcastMessage(reply);
                            break;
                        ...
                        joueurCourant = joueurCourant%4;//(4->0)
                        if(pla_out[joueurCourant] != 1)//until the new joueurCourant stil alive.
                            break;
				...
				break;
                    ...
                    for(int k=0;k<nbClients;k++){
                      if(tableCartes[k][objetSel]!=0){
                      sprintf(reply,"V %d %d %d", k, objetSel, 100);//sprintf(reply,"V %d %d %d", k, objetSel,tableCartes[k][objetSel]);
                      broadcastMessage(reply);
                      }
                      else{
                      sprintf(reply,"V %d %d %d",k, objetSel, 0);
                      broadcastMessage(reply);
                        }    
                    }
                    while(1)
